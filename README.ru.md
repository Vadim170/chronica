...
1. Скачайте `Chronica-<версия>.dmg` из [Releases](https://github.com/Vadim170/chronica/releases) и перетащите приложение в `/Applications`. DMG подписан сертификатом Developer ID и нотаризован Apple, так что Gatekeeper откроет его без плясок с бубном.
...
4. Нажмите кнопку записи. macOS спросит доступ к **микрофону**, а для дорожки системного звука — к **записи экрана и системного звука**. Диалоги появляются только в подписанном приложении. Если доступ к микрофону отклонён, панель покажет кнопку перехода в нужный раздел системных настроек, а не «замолчит».
...
Ядро само не трогает микрофон: оболочка пушит PCM-кадры через `push_audio_frame`, а ядро ресемплит и обрабатывает. То же ядро через UniFFI Kotlin работает в Android-клиенте.
...
android/   Экспериментальный Android-клиент на том же ядре — публичного релиза нет
